...
from .src.tools import init_logger, logger
...
class MMPredTest(nn.Module):
    ...
    def get_test_data(self, file_path, vocab_path, example_path, cached_features_path, batch_size, num_steps):
    ...
    def run_test(self, model_path, test_dataloader, input_length, output_length, device='', save_path=None, topK=10):

        map_location = "cpu" if device == '' else f"cuda:{device}"

        states = torch.load(model_path, map_location=map_location)
        logger.info(f"Load model from {model_path}")
        model = MMPred(SmallConfig)
        model.load_state_dict(states["model"])

        logger.info('model predicting....')

        generator = Generator(model=model, logger=logger, n_gpu=map_location)
